"""Importer for Opera password exports."""
import csv
import json
import logging
import os
...
class OperaImporter(BaseImporter):
    """Importer for Opera passwords."""
    
    def __init__(self):
        super().__init__()
        self.opera_data_path = self._get_opera_data_path()
    
    def _get_opera_data_path(self) -> Path:
        """Get the Opera user data directory path."""
        if os.name == 'nt':  # Windows
            return Path(os.path.expanduser("~")) / "AppData" / "Roaming" / "Opera Software" / "Opera Stable"
        elif os.name == 'posix':  # macOS and Linux
            if os.uname().sysname == 'Darwin':  # macOS
                return Path("~") / "Library/Application Support/com.operasoftware.Opera"
            else:  # Linux
                return Path("~") / ".config/opera"
        return Path(".")
    
    def can_import(self, file_path: str) -> bool:
        """Check if the file is an Opera password export or if Opera data exists."""
        ...
        # Check if Opera's Login Data file exists
        login_data = self.opera_data_path / "Login Data"
        return login_data.exists()
    
    def import_from_file(self, file_path: str, master_password: Optional[str] = None) -> List[PasswordEntry]:
        """
        Import passwords from Opera's data file or CSV export.
        
        Args:
            file_path: Path to the Opera data file or CSV export
            master_password: Not used for Opera (kept for interface compatibility)
    ...
    def _import_from_csv(self, file_path: str) -> List[PasswordEntry]:
        ...
        try:
                ...
                for row in reader:
                    try:
                        entry = PasswordEntry(
                            ...
                            notes=f"Imported from Opera CSV on {self._get_current_timestamp()}",
                            tags=['imported', 'opera']
                        )
            ...
            logger.error(f"Error reading Opera CSV file: {e}")
            self.stats.failed += 1
        
        return entries
    
    def _import_from_sqlite(self, file_path: str) -> List[PasswordEntry]:
        """Import passwords from Opera's SQLite database."""
        entries = []
        temp_db = None
        
        try:
            # Create a temporary copy of the database since Opera locks it
            import tempfile
            import shutil
            ...
            for url, username, encrypted_password, created, last_used, times_used in cursor.fetchall():
                try:
                    ...
                    if encrypted_password.startswith(b'v10') or encrypted_password.startswith(b'v11'):
                        # AES-GCM encryption (Chrome 80+ and Opera 67+)
                    ...
                    entry = PasswordEntry(
                        name=url.split('//')[-1].split('/')[0] if url else "Opera Import",
                        url=url,
                        username=username,
                        password=password,
                        notes=f"Imported from Opera on {self._get_current_timestamp()}\n"
            ...
            logger.error(f"Error reading Opera database: {e}")
            self.stats.failed += 1
        finally:
    ...
    def _get_encryption_key(self) -> bytes:
        """Get the encryption key from Opera's Local State file."""
        local_state_path = self.opera_data_path / "Local State"
        try:
    ...
    def _decrypt_aes_ecb(self, encrypted_data: bytes, key: bytes) -> str:
        """Decrypt data using AES-ECB (old Chrome/Opera versions)."""
    ...
    def _format_timestamp(self, timestamp: int) -> str:
        """Format Chrome/Opera timestamp to a readable string."""
        if not timestamp:
            return "Never"
            
        # Chrome/Opera timestamp is microseconds since Jan 1, 1601
